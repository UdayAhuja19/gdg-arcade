...
test("a phone joins, and the big screen sees it, its dot and its taps", async () => {
  await clearRoom();
  const screen = await openScreen();
  assert.equal((await screen.next((m) => m.t === "join")).maxPlayers, 4);
...
test("the 5th phone is told the party is full", async () => {
  await clearRoom();
  const phones = [];
  for (let i = 0; i < 4; i += 1) phones.push((await joinPhone(`phone-full-${i}`, `P${i}`)).phone);
...
test("a battle starts with fewer than 4 phones, from the host phone or the big screen", async () => {
  await clearRoom();
  const screen = await openScreen();
  ...
  const host = await joinPhone("phone-jjjj-1", "HOST PHONE");
  const hostLobby = await host.phone.next((m) => m.t === "round" && m.players === 1);
  assert.equal(hostLobby.phase, "lobby");
  assert.equal(hostLobby.host, true);
  assert.equal(hostLobby.minPlayers, 1);

  const guest = await joinPhone("phone-jjjj-2", "GUEST PHONE");
  const guestLobby = await guest.phone.next((m) => m.t === "round" && m.players === 2);
  assert.equal(guestLobby.host, false);
  assert.equal(guestLobby.hostName, "HOST PHONE");

  // Only the host phone can start.
  guest.phone.send({ t: "start" });
  await new Promise((resolve) => setTimeout(resolve, 100));
  assert.equal(host.phone.inbox.some((m) => m.t === "round" && m.phase === "countdown"), false);

  host.phone.send({ t: "start" });
  const countdown = await guest.phone.next((m) => m.t === "round" && m.phase === "countdown");
  ...
  assert.deepEqual(state.round.bars.map((b) => b.name).sort(), ["GUEST PHONE", "HOST PHONE"]);

  screen.send({ t: "start" });
  assert.equal((await screen.next((m) => m.t === "notice")).message, "A ROUND IS ALREADY RUNNING.");

  await Promise.all([host.phone.close(), guest.phone.close()]);
  await screen.close();
  await clearRoom();
});
...
test("in a battle, the first full bar wins, reports are confirmed, and a late phone still counts", async () => {
  ...
  try {
    ...
    back.phone.send({ t: "progress", round: 1, seq: 7, level: 100, taps: 66, done: true, ms: 5500, final: true });
