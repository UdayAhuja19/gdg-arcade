...
- **MASH BATTLE rounds** (`round.js`, also pure logic; tests in `test/party-round.test.js`): lobby → countdown (3s) → playing (30s limit) → results.
  ...
  - **MASH BATTLE's rules** Its rules live in `public/js/shared/tap-battle.js` (shared by phone and server): each tap adds `TAP_FILL`, the bar always drains `DRAIN_PER_SEC`, and after `IDLE_AFTER_MS` without a tap it also drains `IDLE_DRAIN_PER_SEC`. First full bar wins; at the time limit, the fullest bar wins.
...
- **SNAKE ROYALE** is the opposite design: **the laptop runs the game and phones are only controllers.** Players watch the big screen, which is local, so the board has no network lag.
  ...
  - **Big screen lobby** (`party/screen.html`, `party/js/screen.js`) is built for spectators: the next game's name as a huge bracketed title with a tagline and 3 steps, a big SCAN TO PLAY QR card, a PREVIEW card where `party/js/lobby-demo.js` plays the next game with bots (real `createArena` rules on the real board renderer for SNAKE ROYALE, four racing bars for MASH BATTLE; it only runs while the lobby is visible and shows a still frame under reduced motion), and a seat per player that fills in their colour. **STATS** (remembered in localStorage) swaps in the per-phone connection cards 2 by 2. The lobby is hidden while a round is on.
...
- **WebSocket protocol:** JSON with a `t` field. The server measures RTT on its own clock.
  ...
  | Direction | Messages |
  |---|---|
  | Phone → server | `join {clientId,name,network,rejoin}`, `pong {id}`, `update {x}` (10Hz), `tap` (lobby), `progress {round,seq,level,taps,done,ms,final}` (10Hz in a MASH BATTLE round), `turn {round,dir}` (SNAKE ROYALE), `start` (host only), `network`, `pause`, `resume`, `leave` |
